...
function normaliseRoute(url) {
  ...
  const $url = new URL(url);
  const path = `${ufo.withLeadingSlash($url.pathname)}${$url.search}`;
  let normalised = {
...
const runLighthouseTask = async (props) => {
  ...
  if (report.fullPageScreenshot?.screenshot.data)
    await fs__default.writeFile(node_path.join(routeReport.artifactPath, ReportArtifacts.fullScreenScreenshot), base64ToBuffer(report.fullPageScreenshot.screenshot.data));
...
async function createUnlighthouseWorker(tasks) {
  ...
  const requeueReport = (report) => {
    ...
    Object.values(ReportArtifacts).forEach((artifact) => {
      fs__default$1.rmSync(node_path.join(report.artifactPath, artifact), { force: true, recursive: true });
    });
...
const resolveUserConfig = async (userConfig) => {
  ...
  config.outputPath = node_path.resolve(config.root, config.outputPath);
  return config;
};
...
async function createUnlighthouse(userConfig, provider) {
  ...
  const { __dirname } = mlly.createCommonJS((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href)));
  ...
  ctx.setCiContext = async () => {
    ...
    ctx.runtimeSettings = {
      ...
      resolvedClientPath: await mlly.resolvePath(ClientPkg, { url: (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href)) })
    };
